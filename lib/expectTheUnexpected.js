...
function expectTheUnexpected(unexpected) {
    ...
    var flags = {
        ...
        with: [],
        only: ['have'],
        have: ['all', 'any', 'deep', 'length', 'own', 'property'],
        contain: ['all', 'any'],
        deep: [],
        be: [],
        an: []
    ...
    function defineSteps(parent, flagNames) {
        for (const prop of flagNames) {
            defineOneStep(parent, prop);
        }
    }

    function defineOneStep(parent, prop, stepFlags) {
        stepFlags = stepFlags || flags[prop];

        Object.defineProperty(parent, prop, {
            get: function() {
                var obj = new ExpectFacade(parent.subject, stepFlags);
                ...
                if (typeof ExpectFacade.prototype[prop] === 'function') {
                    var fn = function(...args) {
                        ...
                        defineOneStep(obj, 'and', flags.to);
                        defineSteps(obj, Object.keys(flags));

                        return obj;
                    };

                    if (flagsAssertions[prop]) {
                        const extraFlags = flagsAssertions[prop];
                        const standardFlags = flags[prop] || [];

                        defineSteps(fn, [...extraFlags, ...standardFlags]);
                    }
